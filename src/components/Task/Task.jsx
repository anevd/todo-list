					...
					<form className={styles.form__edit} onSubmit={handleSubmitEdit}>
						...
						<label className="card-content">
							<input
								...
								value={value}
							/>
							{width <= breakpoint && <button className="waves-effect waves-light btn-small blue lighten-3">✓</button>}
							{width > breakpoint && <button className="waves-effect waves-light btn-small blue lighten-3">Сохранить</button>}
						</label>
					</form>
				) : (
					<div className="card-content">
						<p className={styles.task__text}>
							<label>
								...
								/>
								<span className={status ? styles.resolvedClass : styles.task__text}>{text}</span>
							</label>
						</p>
						<div className={`card-content_icons ${styles.task__icons}`}>
